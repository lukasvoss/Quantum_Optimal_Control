        ...
        try:
            ...
            with Tracker() as tracker:
                for ii in tqdm.tqdm(range(1, total_updates + 1)):
                    if tracker.qpu_tasks_cost() < max_cost:
                ...
                return {
                    ...
                    'action_vector': avg_action_history[np.argmax(fidelities)],
                }
        except Exception as e:
            logging.error(f'An error occurred at iteration {ii} during training: {e}')
            avg_reward = [arr.tolist() for arr in avg_reward] if not isinstance(avg_reward, list) else avg_reward
            std_actions = [arr.tolist() for arr in std_actions] if not isinstance(std_actions, list) else std_actions
            avg_action_history = [arr.tolist() for arr in avg_action_history] if not isinstance(avg_action_history, list) else avg_action_history
            return {
                    'avg_reward': avg_reward,
                    'std_actions': std_actions,
                    'fidelities': fidelities,

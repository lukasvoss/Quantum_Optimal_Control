...
class HardwareRuntime:
    """
    Hardware runtime (on the QPU chip) constraint for training (in seconds). To use this mode, you should ensure that the selected backend
...
class ExperimentRuntime:
    """
    Complete (calibration) experiment runtime of a QPU being the sum of hardware runtime and time used by the control stack (in seconds). 
    To use this mode, you should ensure that the selected backend does have a non-trivial InstructionDurations attribute (the user can set 
    this attribute manually in the config as well).

    :param experiment_runtime: Experiment runtime constraint for training (in seconds)
    """
    experiment_runtime: Union[int, float] = None
